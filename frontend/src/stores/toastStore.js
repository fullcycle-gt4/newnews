...
export const useToastStore = create((set, get) => ({
  ...
  dismissToast: (notificationId) => {
    ...
    set({ toastNotifications: state.toastNotifications.filter((t) => t.id !== notificationId) })
  },

  showToastNotification: (message, type = 'info', iconSymbol) => {
    const notificationId = Date.now() + Math.random()
    set((state) => ({
      toastNotifications: [...state.toastNotifications, { id: notificationId, message, type, icon: iconSymbol }]
    }))
